...
from .box_pushing_cfg import BoxPushingEnvCfg, RandomizationCfg
...
class LiftObservationManager(ObservationManager):
    ...
    def arm_dof_pos_scaled(self, env: BoxPushingEnv):
    ...
    def arm_dof_vel(self, env: BoxPushingEnv):
        """DOF velocity of the arm."""
        return env.robot.data.arm_dof_vel

    def tool_dof_pos_scaled(self, env: BoxPushingEnv):
    ...
    def object_orientations(self, env: BoxPushingEnv):
    ...
    def object_relative_tool_positions(self, env: BoxPushingEnv):
        """Current object position w.r.t. end-effector frame."""
        return env.object.data.root_pos_w - env.robot.data.ee_state_w[:, :3]

    def object_relative_tool_orientations(self, env: BoxPushingEnv):
    ...
    def object_desired_positions(self, env: BoxPushingEnv):
        """Desired object position."""
        return env.object_des_pose_w[:, 0:3] - env.envs_positions

    def object_desired_orientations(self, env: BoxPushingEnv):
    ...
    def arm_actions(self, env: BoxPushingEnv):
        """Last arm actions provided to env."""
        return env.actions[:, :-1]

    def tool_actions(self, env: BoxPushingEnv):
        """Last tool actions provided to env."""
        return env.actions[:, -1].unsqueeze(1)

    def tool_actions_bool(self, env: BoxPushingEnv):
